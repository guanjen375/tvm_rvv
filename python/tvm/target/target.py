...
def my_device(model="my_device", options=None):
    """Returns a my_device target.
    
    This function constructs a Target object for a custom device named "my_device".
    It uses the opencl backend but attaches minimal target properties via options:
      - keys: must include "my_device" and "gpu"
      - device: set as "my_device"
    
    These options ensure that TVM's lowering passes (如 split_host_device) 
    将该目标识别为设备端函数，从而生成类似 CUDA 内核启动的调用包装（call_packed_lowered）。
    
    Parameters
    ----------
    model : str
        A model name标识符，默认为 "my_device"。
    ...
    target : tvm.target.Target
        构造好的 Target 对象。
    """
    # 默认选项：将 keys 设置为 "my_device,gpu" 并明确设备标识
    opts = ["-keys=my_device,cpu", "-device=my_device"]
    opts = _merge_opts(opts, options)
    # 这里基于 opencl 后端构造目标字符串
    target_str = " ".join(["opencl"] + opts)
