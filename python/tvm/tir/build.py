...
def my_device_annotate(mod: IRModule):
    """Annotate to my_device"""
    sch = tir.Schedule(mod)
    for gv in sch.mod.functions.keys():
        # 假設 gv.name_hint 存在且包含 "my_device" 表示這是屬於 my_device 的函式
        if "my_device" in gv.name_hint:
            updated_func = sch.mod.functions[gv] \
                .with_attr("calling_conv", 2) \
                .with_attr("target", Target("my_device")) \
                .with_attr("tvm_global_symbol", gv.name_hint)
            sch.mod.update_func(gv, updated_func)
            print("Updated calling_conv for device function", gv.name_hint)
        
    return mod

def InjectMyDeviceGlobalFuncAttr(mod: IRModule) -> IRModule:
    new_funcs = {}
    for gv, func in mod.functions.items():
        if isinstance(func, tir.PrimFunc):
            target = func.attrs.get("target", None)
            if target is not None:
                # 采用字符串查找方式判断 target 是否属于 my_device
                target_str = str(target)
                if "my_device" in target_str:
                    if "tir.is_global_func" not in func.attrs:
                        print("Injecting tir.is_global_func for", gv.name_hint)
                        func = func.with_attr("tir.is_global_func", tir.const(True, "bool"))
        new_funcs[gv] = func
    return IRModule(new_funcs, attrs=mod.attrs)
...
def build(
    ...
    #mod = InjectMyDeviceGlobalFuncAttr(mod)
    #mod = my_device_annotate(mod)
